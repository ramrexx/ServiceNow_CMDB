=begin
 servicenow_update_cmdb.rb

 Author: Kevin Morey <kevin@redhat.com>

 Description: This method updates a ServiceNow CMDB Record via REST API
-------------------------------------------------------------------------------
...
begin
  ...
  unless body_hash.nil?
    if @task
      ...
      body_hash[:short_description] = "#{@object.name} - CMDB record updated via provision request #{@task.miq_request.id}"
    else
      unless $evm.root['dialog_miq_alert_description'].nil?
        log(:info, "Detected button service dialog driven event with description")
        body_hash[:short_description] = "#{@object.name} - CMDB record manually updated - #{$evm.root['dialog_miq_alert_description']}"
      else
        log(:info, "Detected button service dialog driven event")
        body_hash[:short_description] = "#{@object.name} - CMDB record manually updated"
      end
    end
    ...
    cmdb_result = call_servicenow(:put, get_tablename, servicenow_cmdb_sysid, body_hash)
